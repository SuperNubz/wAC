/*
    change setpointangle on line 290 to your desired home position of the motor.
    change <angle = 200> on line 288 the threshold on how far the motor would compress.
*/
...
unsigned char displayHOLDTIME[8] = {0x5a, 0xa5, 0x06, 0x82, 0x50, 0x00, 0x00, 0x00};
unsigned char display_I[8] = {0x5a, 0xa5, 0x05, 0x82, 0x56, 0x00, 0x00, 0x00};    
unsigned char display_E[8] = {0x5a, 0xa5, 0x05, 0x82, 0x63, 0x00, 0x00, 0x00};          
unsigned char displayBreathsPerMinute[8] = {0x5a, 0xa5, 0x06, 0x82, 0x54, 0x00, 0x00, 0x00};
unsigned char displaySensitivity[8] = {0x5a, 0xa5, 0x06, 0x82, 0x58, 0x00, 0x00, 0x00};
unsigned char displayPEEP[8] = {0x5a, 0xa5, 0x06, 0x82, 0x64, 0x00, 0x00, 0x00};
unsigned char displayPIP[8] = {0x5a, 0xa5, 0x06, 0x82, 0x65, 0x00, 0x00, 0x00};
unsigned char displayPlateau[8] = {0x5a, 0xa5, 0x06, 0x82, 0x66, 0x00, 0x00, 0x00};
...
int pressureMax = 20;   //set the maximum pressureValue here 
//int volume = 0; 
//int finalVolume = 0;
float m;
float b;
...
void loop() 
{
    ...
    if (!IMVswitchRead)
     {
      ...
      //displayVolume_Default();    //displays the volume to zero as default //TANGGALON
  ...
  if (IMVswitchRead) //IMV switch is on
    {
      digitalWrite(36, HIGH);// turn relay ON     
      checkSwitchState();     
        ...
        {          
          if (idealInhaleTime < (Initial_Inhale_Duration + Set_holdTime)) 
          ...
          else 
            {
              ...
              lastSpeed = constrain(lastSpeed, 25, 255); //constrain to minimum and maximum speed of motor to avoid damage //BASIN PULIHAN ANG CONSTRAIN
        }   
      ...
      if (inHome == 0) // Ensuring that assist control feature is available throughout the duration of inHome == 0 / exhale phase
{
    ...
    {
        // Store the previous PEEP value
        float previousPEEP = PEEP;
        
        // Read the new current pressure value and update PEEP
        pressureValue = pressureSensor(); // read the new current value
        pressureValue = movingaverage(pressureValue);
        PEEP2 += pressureValue;
        count += 1;
        PEEP = PEEP2 / count; // Calculate the current PEEP

        Serial.print("Current PEEP: "); Serial.print(PEEP); Serial.print("  |   ");
        Serial.print("Previous PEEP: "); Serial.print(previousPEEP); Serial.print("   |   ");
        Serial.print("Sensitivity: "); Serial.print(sensitivityValue); Serial.print("   |   ");
        Serial.println(previousPEEP - sensitivityValue);

        // Check if the current PEEP has dropped below the previous PEEP by the sensitivity value
        if (PEEP <= previousPEEP - sensitivityValue)
        {
    ...
    if ((inHome == 1 && (pressureValue >= setpointPressure)) || (inHome == 1 && angle >= 135)) //this block will execute when it hits the target pressureValue or reaches the hard limit angle
      {
        ...
        //finalVolume = m * angle + b; // calculate volume at the peak // TANGGALON
       // displayVolumeFunc(finalVolume); // display volume at the peak // TANGGALON
        ...
        while ( actual_holdTime < Set_holdTime) //hold period, measuring plateau pressureValue
            {
              ...
              // Serial.print("actual_holdTime: ");
              // Serial.println(actual_holdTime);
              count += 1;
            }
              // Serial.print("actual_holdTime: ");
              // Serial.println(actual_holdTime);
...
//          Serial.print("Total_Inhale_Duration");
...
int encoder()
{
  ...
  int dataOut = ADS.readADC(3);    
  // dataOut = ADS.readADC(3);  //analog pin for encoder angle is connected at A3 on ADC
  // encoderAngle = map(dataOut, 0, 32767, 0, 359);  // Remap the 16-bit reading to 0-359
  // encoderAngle = map(dataOut, 60, 10842, 0, 359);  // Remap the 16-bit reading to 0-359
  encoderAngle = map(dataOut, 150, 27700, 0, 359);  // Remap the 16-bit reading to 0-359
  // int constrained = (int) constrain(encoderAngle, 0 ,359);
  float encoderAngleFinal = map(encoderAngle, 0, 1023, 0, 359);
//  Serial.println(encoderAngle);
//  Serial.print("Encoder Angle: ");
//  Serial.print(encoderAngle);
//  Serial.print("    |     ");
//  Serial.print(setpointAngle * .95);
//  Serial.print(" | ");
//  Serial.println(setpointAngle * 1.05);
  return encoderAngle;
}
...
float pressureSensor()
{
  ...
  int16_t adc0 = ads.readADC_Differential_0_1();
  float pressure = map(adc0, 7748, 19300, 0, 2000);
  pressure_cmH2O = ((pressure*1.138284)-0.027469)/98.0665;
  
//   Serial.print("Pressure cmH20:   ");
//   Serial.println(pressure_cmH2O);
...
void checkSwitchState() // CHANGES SA HEXADECIMAL ADDRESS TUNGOD SA NEW DISPLAY - CODE SA DWIN
{
  if (Serial2.available()) 
  {
    ...
    if (Buffer[0] == 0x5A) 
    {
      switch (Buffer[4]) 
      {
          ...
          if (millis() - lastStateChangeTime >= debouncePeriod) 
           {
            if (Buffer[8] == 0)
             {
              IMVswitchRead = true; // Update switch state to off
              Serial.println("IMV Switch Turned on");
              lastStateChangeTime = millis();
             } 
            else if (Buffer[8] == 1)
             {
              IMVswitchRead = false; // Update switch state to on
              Serial.println("IMVSwitch Turned off");
              lastStateChangeTime = millis();
             }
          ...
          if (millis() - lastStateChangeTime >= debouncePeriod) 
           {
            if (Buffer[8] == 0)
             {
              assistSwitchRead = true; // Update switch state to off
              Serial.print("Assist Switch Read turned on.");
              lastStateChangeTime = millis();
             } 
            else if (Buffer[8] == 1)
             {
              assistSwitchRead = false; // Update switch state to on
              Serial.print("Assist Switch Read turned off.");
              lastStateChangeTime = millis();
             }
            // Update the last state change time
           }
        ...
        //case 0x73:  // IE control switch // Dapat wala na ni karon
          // Only process this state change if enough time has passed since the last one
          //if (millis() - lastStateChangeTime >= debouncePeriod) 
           //{
            //if (Buffer[8] == 0)
        ...
        //break;
         ...
         case 0x74:  // Hold Time switch     //Wala pani nasulbad haaaaa
          // Only process this state change if enough time has passed since the last one
          if (millis() - lastStateChangeTime >= debouncePeriod) 
           {
            if (Buffer[8] == 0) 
      ...
      switch (Buffer[4]) 
        {
          ...
          // setpointPressure = map(pressureSlider, 0, 25, pressureMin, pressureMax);
...
{
  int  mappedPressure;
  mappedPressure = map (pressureSlider, 0, 20, pressureMin, pressureMax);
  mappedPressure = (int) constrain(mappedPressure, 0, 20);
  setpointPressure = mappedPressure;
  ...
  delay(3);
  Serial2.write(0x06);  // Length: VP address + write command + length of the int (2 bytes)
  delay(3);
  ...
  delay(3);
  Serial2.write(highByte(mappedPressure));
  delay(3);
  Serial2.write(lowByte(mappedPressure));
  // byte hex[4] = {0};
  // FloatToHexPressure(mappedPressure, hex);

  // Serial2.write(hex[3]);delay(3);
  // Serial2.write(hex[2]);delay(3);
  // Serial2.write(hex[1]);delay(3);
  // Serial2.write(hex[0]);delay(3);
...
//void displayVolumeFunc(int volume) // TANGGALON
//{
...
//void displayVolume_Default() // TANGGALON
//{
...
void displayRespiratoryRate()
{
  ...
  int displayBPM;
  displayBPM = map(BPMslider, 0, 18, 5, 18);
  //desiredTotalCycleDuration = map(BPMslider, 0, 18, 12000, 3333);  
  desiredTotalCycleDuration = 60000 / displayBPM;
  desiredTotalCycleDuration = (int) constrain(desiredTotalCycleDuration, 3300, 12000);  
  float unscaledBPM = 60000 / desiredTotalCycleDuration;

  BPMvalue =  (int) unscaledBPM - 1; //the "2" here is an offset based on actual observation/stopwatch.
  ...
  for(int i = 0; i <= 8; i++){
    Serial2.write(displayBreathsPerMinute[i]);
    // Serial.print(displayBreathsPerMinute[i]);
    delay(3);
  }
  // Serial.println();
...
void displaySNTVTYFunc(int sensitivity_slider, bool useDefault)
{
  ...
  {
    mappedSNTVTY = map(sensitivity_slider, 0, 5, 0, 5);
    mappedSNTVTY = constrain(mappedSNTVTY, 0, 5);
  }

  Serial.print("Mapped Sensitivity: ");
  Serial.println(mappedSNTVTY);

  displaySensitivity[6] = highByte(mappedSNTVTY);
  displaySensitivity[7] = lowByte(mappedSNTVTY);
  for(int i = 0; i <= 8; i++){
    Serial2.write(displaySensitivity[i]);
    delay(3);
  }
...
{
   int mappedHOLDTIME;
   mappedHOLDTIME = map(Slider_HoldTime, 0, 232, 0, 1000);

    Serial.print("mappedHOLDTIME: ");
    Serial.println(mappedHOLDTIME);

   displayHOLDTIME[6] = highByte(mappedHOLDTIME);
   displayHOLDTIME[7] = lowByte(mappedHOLDTIME);
   for(int i = 0; i <= 8; i++){
    Serial2.write(displayHOLDTIME[i]);
    delay(3);
   }
...
void displayExhaleTime_for_IE()
{
  ...
  display_E[6] = highByte(E);
  display_E[7] = lowByte(E);

  for(int i = 0; i <= 8; i++){
    Serial2.write(display_E[i]);
    delay(3);
  }
...
void displayExhaleTime_for_IE_Default()
{
  ...
  unsigned long currentMillis = millis();
    ...
    if (currentMillis - previousMillis >= interval) 
    {
      ...
      Serial2.write(0x07);  // Length: VP address + write command + length of the int (2 bytes)
      delay(3);
      Serial2.write(0x83);  // Write command
      delay(3);
      Serial2.write(0x63);  // Write address
      delay(3);
      Serial2.write((byte) 0x00); // Write address
...
void displayPlateauValue(int plateau)
{
  ...
  for(int i = 0; i <= 8; i++){
    Serial2.write(displayPlateau[i]);
    delay(3);
  }
}
