...
unsigned char displayHOLDTIME[8] = {0x5a, 0xa5, 0x06, 0x83, 0x50, 0x00, 0x00, 0x00};
unsigned char display_I[8] = {0x5a, 0xa5, 0x05, 0x82, 0x56, 0x00, 0x00, 0x00};                    
unsigned char displayBreathsPerMinute[8] = {0x5a, 0xa5, 0x06, 0x83, 0x54, 0x00, 0x00, 0x00};
unsigned char displaySensitivity[8] = {0x5a, 0xa5, 0x06, 0x83, 0x58, 0x00, 0x00, 0x00};
unsigned char displayPEEP[8] = {0x5a, 0xa5, 0x06, 0x83, 0x64, 0x00, 0x00, 0x00};
unsigned char displayPIP[8] = {0x5a, 0xa5, 0x06, 0x83, 0x65, 0x00, 0x00, 0x00};
unsigned char displayPlateau[8] = {0x5a, 0xa5, 0x06, 0x83, 0x66, 0x00, 0x00, 0x00};
        ...
        {          
          if (idealInhaleTime < (Initial_Inhale_Duration + Set_holdTime)) 
          ...
          else 
            {
              ...
              lastSpeed = constrain(lastSpeed, 25, 250); //constrain to minimum and maximum speed of motor to avoid damage //BASIN PULIHAN ANG CONSTRAIN
        }   
    ...
    if (inHome == 0)
      {
        readSliders();
        checkSwitchState();
        setpointAngle = 90;  //always set the angle to 90 degrees as a default home position // BASIN PULIHAN
        volume = 0;
      }
...
//this section is the the switching of motor state
    ...
    if ((inHome == 1 && (pressureValue >= setpointPressure)) || (inHome == 1 && angle >= 195)) //this block will execute when it hits the target pressureValue or reaches the hard limit angle
      ...
      else if (inHome == 0 && (encoderAngle >= 0.95 * setpointAngle && encoderAngle <= 1.05 * setpointAngle)) // it will be executed after it hits the default 90 degree home position // +-5% margin of error and 0.95 & 1.05
      {
        readSliders();
        checkSwitchState();
